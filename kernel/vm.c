...
mmap_handler(uint64 va, int scause)
{
  ...
  if(v == 0) return -1; // 该进程无 vma
  if(scause == 13 && !(v->permission & PTE_R)) return -1; // 试图读取不可读的 vma，错误
  if(scause == 15 && !(v->permission & PTE_W)) return -1; // 试图读取不可写的 vma，错误
  ...
  if(mappages(p->pagetable, va, PGSIZE, (uint64)mem, v->permission) != 0){
    kfree(mem);
    return -1; // map page failed
  }

  struct file *f = v->file;
  ilock(f->ip);
  // 从映射的文件中读出数据到新分配的页面中去
...
{
  // 如果文件本身是不可写的或者有MAP_PRIVATE则无须写回
